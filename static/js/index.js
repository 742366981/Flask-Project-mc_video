$(function () {
    ...
    if (localStorage.dy_name) {
        ...
        $.getJSON('/video/movies/?size=10', function(data) {
            $('#focus .pic:eq(1)').add('#focus .pic:eq(5)').attr('href', '/cinema/movie_details/' + data.results[9].id + '/').children().attr('src', data.results[0].movie_img).css({'width': '375x', 'height': '125px'});
